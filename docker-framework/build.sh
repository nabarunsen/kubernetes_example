#!/bin/bash
    ...
    cd ${WORKSPACE}/framework-test
               
    curl -u ${DOCKER_USR}:${DOCKER_PSW} $ARTFACTORY_URL/${GRADLE_REPO}/webservices.war
               
    docker build -t ${DOCKER_REG}/${DOCKER_REPO}/docker-framework:${DOCKER_TAG} ${WORKSPACE}/framework-test || errorExit "Building ${DOCKER_REPO}/${DOCKER_REPO}/docker-framework:${DOCKER_TAG} failed"
               
    docker tag ${DOCKER_REG}/${DOCKER_REPO}/docker-framework:${DOCKER_TAG} ${DOCKER_REG}/${DOCKER_REPO}/docker-framework:latest
